...
public class DiamondMagicParticle extends MagicParticle {
    protected DiamondMagicParticle(ClientWorld level, double xCoord, double yCoord, double zCoord, SpriteProvider spriteSet, double xd, double yd, double zd) {
        super(level, xCoord, yCoord, zCoord, spriteSet, xd, yd, zd);
        
        Vector4f colorVector = MagicColorUtils.PROJECTILE_VECTOR.get(BasicMagicProjectileEntity.MagicProjectileType.DIAMOND);
    ...
    public static class Provider implements ParticleFactory<DefaultParticleType> {
        ...
        public Particle createParticle(DefaultParticleType particleType, ClientWorld level,
            ...
            return new DiamondMagicParticle(level, x, y, z, this.sprites, dx, dy, dz);
        }
    }
}
