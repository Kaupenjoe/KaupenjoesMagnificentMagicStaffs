...
public class EmeraldMagicParticle extends MagicParticle {
    protected EmeraldMagicParticle(ClientWorld level, double xCoord, double yCoord, double zCoord, SpriteProvider spriteSet, double xd, double yd, double zd) {
        super(level, xCoord, yCoord, zCoord, spriteSet, xd, yd, zd);
        
        Vector4f colorVector = MagicColorUtils.PROJECTILE_VECTOR.get(BasicMagicProjectileEntity.MagicProjectileType.EMERALD);
    ...
    public static class Provider implements ParticleFactory<DefaultParticleType> {
        ...
        public Particle createParticle(DefaultParticleType particleType, ClientWorld level,
            ...
            return new EmeraldMagicParticle(level, x, y, z, this.sprites, dx, dy, dz);
        }
    }
}
