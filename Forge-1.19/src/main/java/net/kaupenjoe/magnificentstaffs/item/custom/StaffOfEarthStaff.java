...
public class StaffOfEarthStaff extends Item {
    public StaffOfEarthStaff(Properties properties) {
        super(properties);
    }
    ...
    public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand hand) {
        ...
        if (!level.isClientSide) {
            BouncingProjectileEntity magicProjectile = new BouncingProjectileEntity(level, player);
            magicProjectile.shootFromRotation(player, player.getXRot(), player.getYRot(), 0.0F, 0.35F, 0.25F);
